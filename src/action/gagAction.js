...
export function fetchGagInfo(matchData) {
  return (dispatch) => {  
    ...
    axios.get('http://localhost:3000/getGags?id=57f3a561dcba0f79a91e234b').then((response)=> {
      return dispatch({
        type: 'FETCH_GAG_INFO',
...
export function fetchGagsData() {
      console.log('HEEEEEEEEEEEEEEEEEE')
  return (dispatch) => {  
    axios.get('http://localhost:3000/getGags').then((response)=> {
      return dispatch({
...
export function uploadGag(gagData) {
  ...
  return (dispatch) => {  
    axios.post('http://localhost:3000/addGags',gagData).then((response)=> {
